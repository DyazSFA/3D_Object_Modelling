...
void ButtonDraw(Button *b)
{
    ...
    if(b)
    {
         ...
         */
        if (b->highlighted)
            glColor3f(1.0f,0.1f,0.9f); //jika kena hover mouse
        else
            glColor3f(0.0f,1.0f,1.0f); //warna block
...
void profilpict()
{
    ...
    Image* image3 = loadBMP("about.bmp");
...
void drawModel()
{
    if (!monkey)
    {
        monkey = glmReadOBJ("RumahAceh.obj");
        if (!monkey) exit(0);
        glmUnitize(monkey);
...
void Draw()
{
    ...
    if(A==0)
    {
        ...
        glLoadIdentity();
        glColor3f(1,1,0);
        drawBitmapText1("Mengenal Aceh",230,70,0);
...
//    glutDisplayFunc(display);
    ...
    sndPlaySound("MARS UM.wav",SND_ASYNC);
    glutMainLoop();
    return 0;
}
